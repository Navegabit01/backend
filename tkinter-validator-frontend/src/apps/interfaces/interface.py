...
class AmbitApp(ctk.CTk):
    def __init__(self):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Hasta que no funcione el selector de hotkey usare el selector <Control-x>\n",
            font=("Roboto", 12),
        )
        self.label.grid(row=31, column=0, padx=10, pady=(10, 0), sticky="w")

        self.opciones_var1 = ctk.StringVar(value="Texto - Texto")
        self.optionmenu = ctk.CTkOptionMenu(
            self,
            values=["Texto - Texto", "Texto - Audio", "Audio - Texto", "Audio - Audio"],
    ...
    def optionmenu_callback(self, choice):
        if self.options_pos == 3:
            self.options_pos = -1
        self.options_pos += 1
        self.optionmenu.set(self.options[self.options_pos])
